...
class UpdateMaker():
    ...
    def __init__(self, board: str):
        ...
        if self._status_file == '':
            raise FileNotFoundError(f'No status file for {board} board in cache')
    ...
    def add_packages(self,
        ...
        for pkg in packages:
            ...
            if pkg_obj.name in reinstall_not_removed:
                ...
                pkg_obj.mark_install()
            elif pkg_obj.name not in self.not_installed_yet and \
                    pkg_obj.name not in self.not_removed_yet:
                pkg_obj.mark_install()
    ...
    def add_support_files(self, support_files: list):
        ...
        for s_file in support_files:
            if isfile(s_file):
                raise ValueError(f' {s_file} was not found')
    ...
    def make_update_archive(self):
        ...
        if command == 'Y' or command == 'y' or command == 'yes':
            try:
                ...
                print(f'{update_file} was added to update cache')
